...
### Поиск свободных аудиторий (web-first)

**GET** `/api/free-auditories/slots`
- Параметры: `date` (YYYY-MM-DD), `building` (опционально)
- Возвращает динамические слоты (`HH:MM-HH:MM`), реально присутствующие в БД на дату.

**GET** `/api/free-auditories/by-slot`
- Параметры: `date`, `building`, `slot`, `auditory_type` (опционально)
- Возвращает список свободных аудиторий на выбранный слот.

**GET** `/api/free-auditories/by-room`
- Параметры: `date`, `auditory`, `building` (опционально)
- Возвращает свободные/занятые слоты конкретной аудитории на дату.
...
Модуль [`app_manager/`](app_manager/) — загрузка и раздача Android-сборок. Подробнее: [`app_manager/README.md`](app_manager/README.md).

Кратко: скопируйте [`app_manager/config.example.json`](app_manager/config.example.json) в `app_manager/config.json`, положите публичный ключ, загружайте ZIP через `app_manager/scripts/`.
...
Версия бота: **2.0** (в `user_agent` и в логах). Формат версии **X.Y**: первое число — мажор (миграции БД, изменения схемы или компонентов БД), второе — минор (исправления, устранение несоответствий).
...
**Доступность Telegram и прокси (воркер):** при запуске проверяется `https://api.telegram.org/` без прокси; если недоступно и `TELEGRAM_PROXY` не задан — в лог пишется предупреждение включить прокси в конфиге. Если прокси задан — проверяется доступ к API через него. Раз в час — повторная проверка API (через прокси, если он используется). Раз в минуту — только при активном прокси: запрос через текущий агент; при сбое агент пересоздаётся (переподключение к SOCKS/HTTP-прокси без перезапуска процесса). Интервалы можно переопределить: `TG_PROXY_HEALTH_INTERVAL_MS` (по умолчанию 60000), `TG_TELEGRAM_CHECK_INTERVAL_MS` (по умолчанию 3600000), опционально URL проверки `TELEGRAM_PROBE_URL`.
...
- **Личные сообщения:** подписка на несколько сущностей, настройка времени рассылки, просмотр и удаление подписок. В списке «Мои подписки» у каждой подписки можно включить **рассылку без звука** (🔕) — только для ежедневной рассылки по расписанию; inline и ручные запросы не затрагиваются.
- **Тихая рассылка:** для каждой подписки отдельно (в ЛС и в группе через `/removesubs`) переключатель 🔕/🔔. При включённом режиме бот отправляет расписание с `disable_notification` (без звука уведомления в Telegram).

**Структура:** `tgbot/worker.js`, `tgbot/connectivity.js`, `tgbot/config.loader.js`, `tgbot/config.example.json`, `tgbot/handlers/group.js`, `tgbot/handlers/inline.js`, `tgbot/handlers/private.js`, `tgbot/jobs/daily.js`, `tgbot/strings.js`, `tgbot/payload.js`, `tgbot/lists.js`. Рабочий конфиг — `tgbot/config.json` (создаётся вручную из примера, в git не попадает).
...
Версия задаётся переменной `PLAPSER_TG_BOT_INTEGRATION_VERSION` в `tgbot/worker.js`.
